...
class BrushConfig:
    ...
    def __init__(self, config: dict, process_site_config=True):
        ...
        self.media_filter = config.get("media_filter")
        self.resource_filter = config.get("resource_filter")
...
class BrushFlowPlus(_PluginBase):
    ...
    # 插件名称
    plugin_name = "站点刷流Plus"
    # 插件描述
    plugin_desc = "在官方刷流插件基础上，增加媒体、资源过滤功能。"
    # 插件图标
    ...
    @property
    def chain(self) -> TorrentsChain:
        """ 获取种子处理链 """
        return self.get_service(TorrentsChain)
    ...
    def __evaluate_conditions_for_brush(self, torrent, torrent_tasks) -> Tuple[bool, Optional[str]]:
        ...
        # 媒体过滤
        if brush_config.media_filter:
            media_info = self.chain.recognize_media(torrent.title)
            if not media_info:
                return False, "未识别到媒体信息"
            if not self.chain.filter_media(media_info, brush_config.media_filter):
                return False, "不符合媒体过滤规则"

        # 资源过滤
        if brush_config.resource_filter:
            if not self.chain.filter_resource(torrent, brush_config.resource_filter):
                return False, "不符合资源过滤规则"
